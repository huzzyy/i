...
class categoryClass:
    def __init__(self,root):
        ...
        btn_delete=Button(self.root,text='DELETE',command=self.delete,font=("goudy old style",15),bg="red",fg='white',cursor="hand2").place(x=520,y=170,width=150,height=30)
         ...
         ##CATEGORY TREE VIEW DETAILS#@###
        cat_fram=Frame(self.root,bd=3,relief=RIDGE)
        cat_fram.place(x=750,y=100,width=380,height=100)
        
        scrolly=Scrollbar(cat_fram,orient=VERTICAL)
        scrollx=Scrollbar(cat_fram,orient=HORIZONTAL)
        
        
        self.category_table=ttk.Treeview(cat_fram,columns=("cid","name"),yscrollcommand=scrolly.set,xscrollcommand=scrollx.set)
        ...
        self.im1=self.im1.resize((500,300), resample=Image.BILINEAR)
        ...
        self.im2=self.im2.resize((500,300), resample=Image.BILINEAR)
